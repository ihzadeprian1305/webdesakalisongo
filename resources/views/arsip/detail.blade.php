...
<style>
  header{
    ...
    background-image: url('{{ asset('public/cover/'.$DataArsip->cover)}}');
  }
...
<header class="d-flex flex-column mx-5 mt-5">
<nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-primary">
            <div class="container">
            <a class="text-white navbar-brand" href="{{url('/arsip')}}"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
...
@section('main-content')
    <div class="page-content">
      <div>
          <div class="container">
            <div class="h3 pb-3 text-center">{{$DataArsip->judul}}</div>
            <p class="px-5 pb-5 text-center"><i class="fas fa-user"></i> {{$DataArsip->users_updated_at->name}} &nbsp;<i class="fas fa-calendar-alt"></i> {{$DataArsip->updated_at}}</p>
          ...
          <div class="row justify-content-center mt-5">
            <a href="{{url('/arsip/'.$DataArsip->file)}}" class="btn btn-primary text-decoration-none">Download</a>
          </div>
          </div>
          <div class="container mt-3">
            <div class="h3 py-5 text-center">Arsip Terbaru</div>
            <div class="col">
            <div class="row">
            @if (!empty($DataArsipTerbaru))
          @foreach ($DataArsipTerbaru as $key => $Archive )
          <div class="col-md-4">
          <div class="card mb-3">
            <img src="{{ asset('public/cover/'.$Archive->cover) }}" class="card-equal-height-image rounded" alt="">
            <div class="card-body mt-1">
              <div class="h4">
              {{Str::limit($Archive->judul, 40, '...')}}
              </div>
              <p><i class="fas fa-user"></i> {{$Archive->users_updated_at->name}} &nbsp;<i class="fas fa-calendar-alt"></i> {{$Archive->updated_at}}</p>
            </div>
            <div class="col">
              <a href="{{url('/arsip.'.$Archive->id_archive.'.detail')}}" class="text-decoration-none text-white btn btn-primary mx-auto d-block mb-3">Baca</a>
            </div>
          </div>
        </div>  
        @endforeach
        ...
        <div class="col">
      <button type="button" class="btn btn-primary mx-auto d-block mt-3"><a href="{{url('/arsip')}}" class="text-decoration-none text-white">Arsip Lainnya</a></button>
    </div>
            </div>
          </div>
            </div>
